...
public class HeighteningShard extends AbstractShard {

    public HeighteningShard(Properties properties) {
        super(properties);
    }
    ...
    public boolean tryApply(ServerLevel level, Player player, ItemStack target) {
        ...
        if (data.rarity() != ItemRarity.MAGIC) return false;
        if (!isModifiable(data)) return false;

        // Upgrade rarity first, then add one affix
        var registryAccess = level.registryAccess();
        AffixData upgraded = data.withRarity(ItemRarity.RARE);
        AffixData newData = AffixRoller.addRandomAffix(
                registryAccess,
                target,
                upgraded,
                level.getRandom()
